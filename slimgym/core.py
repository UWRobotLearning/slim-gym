...
class Env(Generic[ObsType, ActType]):
    ...
    - :attr:`spec` - An environment spec that contains the information used to initialise the environment from `slimgym.make`
    ...
    def unwrapped(self) -> "Env":
        ...
        Returns:
            Env: The base non-wrapped slimgym.Env instance
        """
        return self
...
class ObservationWrapper(Wrapper):
    ...
    observation wrapper like this::
        ...
        class RelativePosition(slimgym.ObservationWrapper):
...
class RewardWrapper(Wrapper):
    ...
    To do that, we could, for instance, implement the following wrapper::
        ...
        class ClipReward(slimgym.RewardWrapper):
...
class ActionWrapper(Wrapper):
    ...
    Let’s say you have an environment with action space of type :class:`slimgym.spaces.Box`, but you would only like
    to use a finite subset of actions. Then, you might want to implement the following wrapper::

        class DiscreteActions(slimgym.ActionWrapper):
        ...
        if __name__ == "__main__":
            env = slimgym.make("LunarLanderContinuous-v2")
